...
diff_push() {
  ...
  diff_exit_code=0
  flux diff artifact oci://${artifact_name} --insecure-registry \
  ...
  if [[  ${diff_exit_code} -ne 0 ]]; then
    flux_output=$(flux push artifact oci://${artifact_name} \
      ...
      --insecure-registry 2>&1) || exit_code=$?
...
diff_push "${registry}/flux-infra-sync:local" "${kubernetes_path}/infrastructure"
